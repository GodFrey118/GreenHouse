...
function regionInit_1(divId)
{
    $("#" + divId + " > select").change(regionChange_1); // select的onchange事件
    $("#" + divId + " > input:button[class='edit_region']").click(regionEdit); // 编辑按钮的onclick事件
}
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                if (data.retval.length > 0)
                {
                    ...
                    {
                        $(_self).next("select").append("<option id='" + data[i].region_id + "' value='" + data[i].region_id + "'>" + data[i].region_name + "</option>");
                    }
                }
            }
            else
...
/* 地区选择函数 */
function regionInit3(divId3)
{
    $("#" + divId3 + " > select").change(regionChange3); // select的onchange事件
    $("#" + divId3 + " > input:button[class='edit_region']").click(regionEdit); // 编辑按钮的onclick事件
}
    ...
    if (this.value > 0)
    {
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                if (data.retval.length > 0)
                {
                    $("<select><option>" + '请选择...' + "</option></select>").change(regionChange_1).insertAfter(_self);
                    var data  = data.retval;
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("select").append("<option value='" + data[i].region_id + "'>" + data[i].region_name + "</option>");
...
/* 后台商品分类选择函数 */
function gcategoryInitb(divId)
{    
    $("#" + divId + " > select").get(0).onchange = gcategoryChangeb; // select的onchange事件
    window.onerror = function(){return true;}; //屏蔽jquery报错
    $("#" + divId + " .edit_gcategory").click(gcategoryEdit); // 编辑按钮的onclick事件
}

function gcategoryChangeb()
{                       
    // 删除后面的select
    $(this).nextAll("select").remove();
    ...
    if (this.value > 0)
    {
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                if (data.retval.length > 0)
                {  
                    $("<select><option>" + lang.select_pls + "</option></select>").change(gcategoryChangeb).insertAfter(_self);
                    var data  = data.retval;
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("select").append("<option value='" + data[i].cate_id + "'>" + data[i].cate_name + "</option>");
...
/* 商品分类选择函数 */
function gcategoryInits(divId)
{        
    $("#" + divId + " > select").get(0).onchange = gcategoryChanges; // select的onchange事件
    window.onerror = function(){return true;}; //屏蔽jquery报错
    $("#" + divId + " .edit_gcategory").click(gcategoryEdit); // 编辑按钮的onclick事件
}

function gcategoryChanges()
{        
    // 删除后面的select
    $(this).nextAll("select").remove();
    ...
    if (this.value > 0)
    {
        ...
        $.getJSON(url, {'pid':this.value}, function(data){
            if (data.done)
            {
                if (data.retval.length > 0)
                {
                    $("<select><option>" + '请选择...' + "</option></select>").change(gcategoryChanges).insertAfter(_self);
                    var data  = data.retval;
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("select").append("<option value='" + data[i].cate_id + "'>" + data[i].cate_name + "</option>");
